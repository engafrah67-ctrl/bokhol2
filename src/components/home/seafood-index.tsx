        ...
        <div className="lg:col-span-4 space-y-5">
          <div>
            <p className="text-xs font-semibold text-muted-foreground mb-1">
              {t('index_current_price')} ({currentCountry.name})
            </p>
            ...
            >
              {isUp ? '▲' : '▼'} {Math.abs(activeSpecies.change)}% {t('index_this_week')}
            </span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            {[
              { label: t('index_weekly_high'),  value: `${activeSpecies.currency === 'EUR' ? 'EUR' : '$'} ${activeSpecies.weekHigh.toFixed(2)}` },
              { label: t('index_weekly_low'),   value: `${activeSpecies.currency === 'EUR' ? 'EUR' : '$'} ${activeSpecies.weekLow.toFixed(2)}` },
              { label: t('index_source'),       value: currentCountry.source },
              { label: t('index_components'),   value: `${currentCountry.species.length} species` },
            ].map(({ label, value }) => (
