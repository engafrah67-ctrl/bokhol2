...
import { ArrowUpRight, Fish, Lock } from 'lucide-react'
  ...
  return (
    <div className="py-8 px-8 bg-background text-foreground transition-all border border-border rounded-2xl shadow-sm">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <div>
            <div className="flex items-center gap-2.5">
              <h3 className="text-base font-extrabold text-foreground tracking-tight">{t('top_products_title')}</h3>
              {!isLoggedIn && (
                <span className="inline-flex items-center gap-1 text-[10px] font-bold text-amber-700 bg-amber-50 dark:bg-amber-950/40 border border-amber-200/80 dark:border-amber-800/60 px-2 py-0.5 rounded-md">
                  <Lock className="w-2.5 h-2.5" /> {t('top_products_members_only')}
                </span>
              )}
            </div>
        ...
        <Link
          href="/products"
          className="text-xs font-bold text-foreground hover:text-foreground/80 transition-colors inline-flex items-center gap-1 bg-muted px-3 py-1.5 rounded-lg border border-border"
        >
          {t('top_products_view_all')}
        ...
        <table className="w-full text-left text-sm">
          <thead>
            <tr className="border-b border-slate-200 dark:border-slate-800 text-xs text-slate-500 font-medium">
              <th className="py-4 px-4 w-32">{t('top_products_fish')}</th>
              <th className="py-4 px-4">{t('top_products_name')}</th>
              <th className="py-4 px-4">{t('top_products_country')}</th>
              <th className="py-4 px-4">{t('top_products_avg_price')}</th>
              <th className="py-4 px-4 text-right">{t('top_products_suppliers')}</th>
            </tr>
          </thead>
            ...
            {productsList.map((p) => (
              <tr key={p.name} className="hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors group cursor-pointer">
                ...
                <td className="py-4 px-4 font-bold text-slate-900 dark:text-white">
                  <BlurGate tooltipText={t('top_products_lock_price_tooltip')} showLockIcon={true}>
                ...
                <td className="py-4 px-4 text-right">
                  <BlurGate tooltipText={t('top_products_lock_suppliers_tooltip')} showLockIcon={false}>
                    <span className="inline-flex items-center text-xs font-semibold text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 px-3 py-1.5 rounded-full">
                      ...
                      <span className="ml-1">{t('top_products_suppliers')}</span>
                    </span>
                  </BlurGate>
                </td>
              </tr>
