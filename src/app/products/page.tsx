...
import { getAllMarketProducts, type MarketProductCard } from '@/lib/data/products-data'
...
export default function ProductsPage() {
  ...
  const [products, setProducts] = useState<MarketProductCard[]>([])

  useEffect(() => {
    const loadProducts = () => {
      const data = getAllMarketProducts()
      setProducts(data)
    }

    loadProducts()

    // Re-load when localStorage changes (new supplier post)
      ...
      >
        <div className="relative z-10 max-w-3xl mx-auto px-4 flex flex-col items-center">
          ...
          <p className="text-slate-500 text-base mt-3 max-w-xl">
            Browse live supplier offers across {products.length} species. Updated weekly by verified exporters worldwide.
          </p>
