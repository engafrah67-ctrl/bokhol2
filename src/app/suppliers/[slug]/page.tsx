...
export default function SupplierProfilePage() {
  ...
  const [storedCompany, setStoredCompany] = useState<CompanyProfile | null>(null)
  ...
  const loadProfile = async () => {
    ...
    setLoading(true)
    try {
      // 1. Fetch company by slug from Supabase
      ...
      if (companyData) {
        ...
        if (cpData && cpData.length > 0) {
          ...
          setProducts(DEFAULT_SUPPLIER_PRODUCTS)
        }
      } else {
        // Fallback to local stored companies
        const stored = getStoredCompanies().find((c) => c.slug === slug)
        if (stored) {
          setStoredCompany(stored)
          setCompany({
            id: stored.id,
            name: stored.name,
            slug: stored.slug,
            ...
            banner_color: stored.bannerColor,
            status: stored.status,
            is_verified: stored.isVerified,
            activity_score: stored.completenessScore,
            trust_score: stored.isVerified ? 92 : 75,
            country: { name: stored.country, flag_emoji: '', slug: stored.country.toLowerCase() },
                ...
                <div className="flex items-center gap-2 flex-shrink-0 pt-1">
                  {company.status === 'unclaimed' && (
                    <Button
                      ...
                      className="bg-white hover:bg-slate-50 text-[#022B96] border-2 border-[#022B96] text-sm font-bold px-4 py-2.5 rounded-xl shadow-xs cursor-pointer flex items-center gap-1.5 transition-all"
                    >
                  ...
                  {company.status === 'claim_requested' && (
                    <span className="bg-amber-50 border border-amber-200 text-amber-800 text-xs font-bold px-3.5 py-2.5 rounded-xl flex items-center gap-1.5 shadow-xs">
      ...
      <ClaimProfileModal
        company={
          storedCompany || {
            ...
            country: company.country?.name || 'International',
            countryCode: '',
